...
class Flags:
    ...
    def generate_clangd_file(self, stream):
        _clangd_dict = {"CompileFlags": {"Add": self._add, "Remove": self._remove}}
...
def main():
    ...
    for root, dirs, files in os.walk(root_dir, followlinks=False):
        ...
        for name in dirs:
            if name in INCLUDE_FOLDERS:
                flags.push_add_flag(
                    f"-I{pathlib.Path(os.path.join(root, name)).resolve()}"
                )
    ...
    for env_var in INCLUDE_PATH_ENV_VARS:
        env_flags = [
            f"-I{x.strip()}"
            for x in re.split(",|:| ", os.environ.get(env_var, ""))
            if x.strip() != ""
        ]
    ...
    print(
        f"searching for additional manual flags defined in {manual_flags_path}... ",
        end="",
    )
